...
def handle_regular_caller(caller, callType=None):
    ...
    #vivekp/10-06-26/added for testing
    dsh_utils.give_news('caller: %s ,  callType: %s' % \
    					(caller,callType), logging.info)
    ##################################
    ...
    #vivekp/10-06-26/added for testing
    dsh_utils.give_news(pleaseRecord, logging.info)
    ################################
...
def conference_call_init(caller, unknownOrg, unknownPerson):
    """called by handle_staff_caller().
    ...
    dsh_common_agi.conference_call_init(fileName, fileFormat)
...
def conference_call_join(caller, unknownOrg, unknownPerson):
    """called by handle_staff_caller().
    initially modeled after forward_outgoing_call()"""
    
    dsh_common_agi.conference_call_join()
    hangup_signal_handler(0, None)
    return res
...
def handle_staff_caller(caller, unknownOrg, unknownPerson):
    ...
    while True:
        ...
        if choice == '1':
            dsh_utils.give_news('handle_staff_caller -> handle_regular_caller',
        			logging.info)
...
#           return result
        ...
        if choice == '4':
            callee = get_call_num(
                ...
                dv2.db.models,
                get_session())
            result = dsh_common_agi.make_dot_call_file(
            	callee,
                get_session())
        ...
        if choice == '7':
            result = conference_call_join(caller, unknownOrg, unknownPerson)
            return result

        if choice == '8':
            result = conference_call_init(caller, unknownOrg, unknownPerson)
            return result

        if choice == '9':
            result = handle_sub_menu_test(caller, unknownOrg, unknownPerson)
            return result
...
def handle_sub_menu_test(caller, unknownOrg, unknownPerson):
    """
    10/05/26:
    sub-menu for staff caller, for testing.
    """

    count = 0
    staffChoicePrompt = os.path.join(
        dsh_config.lookup('DSH_PROMPT_DIR'),
        dsh_config.lookup('DSH_PROMPT_ENTER_CHOICE_SUBTEST'))
    timeOut1 = dsh_config.lookup('DSH_PROMPT_WAIT1')
    maxRetries = dsh_common_config.lookup2('STAFF_RETRIES')
...
def get_call_num(caller, unknownOrg, unknownPerson, models, sessionID):
    """
    vivekp
    26/06/10: called by dsh_django2.handle_staff_caller().
    get the number entered in the key pad. 
    """	
    funcName = 'dsh_django2.get_call_num:'
    timeOut11 = dsh_config.lookup('DSH_PROMPT_WAIT11')
    voicePrompt = os.path.join(
                  dsh_config.lookup('DSH_PROMPT_DIR'),
                  dsh_config.lookup('DSH_PROMPT_PHONE_NUMBER'))
    ...
    demoSending = dsh_common_config.lookup('DSH_PROMPT_DEMO_DIAL_IN8')
    demoSending = os.path.join(dsh_common_config.lookup('DSH_PROMPT_DIR'),
                               demoSending)
    ...
    while True:
        ...
        # get the callee number from the caller.
        #
        callNum = dsh_agi.get_digits(voicePrompt, 
                                     11, timeOut=timeOut11)
        if callNum == None or callNum == '':
            continue
        if callNum == '*':
            say_goodbye()
